        ...
        <div class="two-col">
          ...
          <div class="skills">
            ...
            <div class="skills-grid">
              <div class="skill-item" v-for="item in 4">
                <div class="skill-icon" :style="{ backgroundColor: skillBgColor }">
                  <img v-if="thisCharacter?.spell"
                    :src="`/images/characters/${thisCharacter.spell}/skills/${thisCharacter.spell}_skill_${item}.png`"
                    alt="">
                </div>
                <div class="skill-meta">
                  <div class="skill-name">{{ thisCharacterSkills[item - 1]?.skill_name }}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="comment-card">
          ...
          <div class="comment-card-footer">
            <div class="comment-card-reply">
              <textarea name="" id="" class="coment-card-reply-textarea" placeholder="请输入评论" maxlength="1024" rows="4"></textarea>
...
onMounted(async () => {
  // 获取指定id的角色
  const res = await axios.get('/api/characters/characterDetail', {
    params: {
      id: characterId
    }
  })
  ...
  // 获取指定角色台词和介绍
  const dialogues = await axios.get('/api/characters/characterDialogues', {
    params: {
      id: characterId
    }
  })
  ...
  // 获取指定角色属性信息
  const stats = await axios.get('/api/characters/stats', {
    params: {
      id: characterId
    }
  })
  ...
  // 获取指定角色技能信息
  const skills = await axios.get('/api/characters/skill', {
    params: {
      id: characterId
    }
  })
...
const skillBgColor = computed(() => {
  if (!thisCharacter.value) return ''
  return propertyColorMap[thisCharacter.value.damage_type]
})
